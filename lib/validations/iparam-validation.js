...
function ValidateKeys(lang, langcontent){
  ...
  for(var key in langcontent){
    ...
    if (!(DEFAULT_VALUE in langcontent[key]))
      err.push(`default_value must be specified for type dropdown in iparam_${lang}.yml`);
    if (!(OPTIONS in langcontent[key])) {
      err.push(`Options must be specified for type dropdown in iparam_${lang}.yml`);
      continue;
    }
    if(!Array.isArray(langcontent[key][OPTIONS])){
      err.push(`Dropdown options for key ${key} in iparam_${lang}.yml is not a list`); 
      continue;
    }
    if(langcontent[key][DEFAULT_VALUE] && !(_.contains(langcontent[key][OPTIONS], langcontent[key][DEFAULT_VALUE])))
      err.push(`Default value specified in iparam_${lang}.yml is not available in options`);
  }
  return err;
}
...
function checkValidKeys(lang, content) {
  ...
  for(var key in content){
    ...
    if(!(_.isObject(content[key])) || missingKeys.length > 0){
      errs.push(`Mandatory key(s) ${missingKeys} missing for ${key} in iparam_${lang}.yml`);
    }
    if(!(content[key][TYPE] == TEXT || content[key][TYPE] == DROPDOWN)) 
      errs.push(`Invalid type '${content[key][TYPE]}' found in iparam_${lang}.yml.`);
    ...
    if(content[key][TYPE] == TEXT) {
      if (OPTIONS in content[key] || DEFAULT_VALUE in content[key]) {
        errs.push(`Options or Default value must not be specified for type text in iparam_${lang}.yml`);
      }
    }
  }
  return errs;
