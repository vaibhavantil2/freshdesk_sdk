  ...
  for(var i in langs){
    ...
    if(content) {
      ...
      for(var key in content){
        if((content[key][TYPE] == DROPDOWN) && (!Array.isArray(content[key][OPTIONS]))) {
          return (`Dropdown options for key ${key} in iparam_${lang}.yml is not a list`);
        }
      }
      var keys = Object.keys(content);
      for(var j in keys){
        if(content[keys[j]][TYPE] == DROPDOWN && content[keys[j]][DEFAULT_VALUE] != undefined){
          if(!(_.contains(content[keys[j]][OPTIONS],content[keys[j]][DEFAULT_VALUE]))){
            errFiles.push(`iparam_${langs[i]}.yml`);
          }
        }
      }
