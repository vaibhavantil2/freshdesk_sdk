...
function ValidateDropdownKeys(langcontent, callback){
  async.forEachOf(langcontent, function(val, key, contentCB) {
    if((val[validationConst.TYPE] == validationConst.DROPDOWN) && (val[validationConst.OPTIONS] == null))
...
function checkValidKeys(lang, content, callback){
  ...
  async.forEachOf(content,function(value, key, asyncCB){
    ...
    if(_.difference(Object.keys(value),validationConst.VALID_FILES ).length > 0){
      return callback(new Error(`Invalid keys in iparam_${lang}.yml `));
    }

    if(!(value[validationConst.TYPE]== validationConst.DROPDOWN || value[validationConst.TYPE] == validationConst.TEXT)){
      return callback(new Error(`Invalid type in iparam_${lang}.yml `));
    }

    if(value[validationConst.TYPE] == validationConst.TEXT) {
      if (validationConst.OPTIONS in value || validationConst.DEFAULT_VALUE in value) {
        return callback(new Error(`Options or Default value must not be specified in iparam_${lang}.yml`))
      }
    }

    if(value[validationConst.TYPE] == validationConst.DROPDOWN) {
      if (!(validationConst.OPTIONS in value)) {
