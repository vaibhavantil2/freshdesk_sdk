...
function readFile(file, callback) {
  fs.readFile(file, manifest.charset, (err, data) => {
    if (err) {
      eh.error(new Error("Error while reading " + file));
    }
    callback(data);
  });
}
...
function createTmpFile(callback){
  fs.ensureFile(tmpFilePath, function (err) {
    if(err)
      eh.error(new Error("Error while creating temp file."));
    callback(null);
  });
}
...
function writeCssToTmpFile(callback){
  getScssContent(function(data) {
    ...
    fs.writeFile(tmpFilePath, toWrite, function(err){
      if(err)
        eh.error(new Error("Error while writing tmp file"));
      callback(null);
    })
  });
}
