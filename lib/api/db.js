...
function normalizeItem(data) {
  var dupData = {};
  for (var key in data) {
    if (!_.isEmpty(key) && !_.isEmpty(data[key]) && !_.isUndefined(data[key])) {
...
function validateKey(dbKey, data) {
  if (_.isEmpty(dbKey)) {
    throw new customError({"message": "The key cannot be blank", "code": INVALID_DATA_STATUS});
  }
  if (_.isEmpty(data)) {
    throw new customError({"message": "The value cannot be blank and should be of type JSON", "code": INVALID_DATA_STATUS});
  }
  if (!_.isObject(data)) {
    throw new customError({"message": "The value cannot be blank and should be of type JSON", "code": UNPROCESSABLE_DATA_STATUS});
  }
  if (dbKey.length > KEY_LENGTH || checkDataSize(data)) {
    throw new customError({"message": "Key length should not exceed 30 characters", "code": INVALID_DATA_STATUS});
  }
  if (_.size(data) > DATA_SIZE) {
    throw new customError({"message": "The value (which is a JSON object) should not contain more than 10 attributes", "code": INVALID_DATA_STATUS});
  }
  if (objsize(data)/1024 > 2) {
    throw new customError({"message": "The combined size of the key and value should not exceed 1KB", "code": INVALID_DATA_STATUS});
  }
}
